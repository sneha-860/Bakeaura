...
export function SellerStorefrontPage() {
  ...
  return <div className="page"><section className="page-hero compact-hero"><p className="eyebrow">Seller storefront</p><h1>{seller?.name || 'Bakeaura seller'}</h1><p>{seller?.email}</p></section><section className="section"><h2>Products</h2>{products.length ? <div className="grid product-grid">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div> : <EmptyState title="No products listed" />}</section><section className="section"><h2>Feed</h2><div className="grid story-grid">{feed.map((item) => <article className="story-card" key={item.id}><ProductImage src={item.imageUrl} /><div><span className="pill">{item.type}</span><p>{item.caption}</p></div></article>)}</div></section></div>;
}

export function InfluencersPage() {
  const [influencers, setInfluencers] = useState([]);
  useEffect(() => { influencersApi.list().then(setInfluencers).catch(() => setInfluencers([])); }, []);
  return <Directory title="Creator discovery" eyebrow="Influencers" icon={<UsersRound size={16} />}>{influencers.map((creator) => <Link className="person-card" key={creator.id} to={`/influencers/${creator.id}`}><span className="avatar">{creator.name?.charAt(0)}</span><strong>{creator.name}</strong><small>{creator.email}</small></Link>)}</Directory>;
}
...
export function InfluencerProfilePage() {
  ...
  return <div className="page"><section className="page-hero compact-hero"><p className="eyebrow">Creator profile</p><h1>{creator?.name || 'Influencer'}</h1><p>{creator?.email}</p></section><div className="grid story-grid">{feed.map((item) => <article className="story-card" key={item.id}><ProductImage src={item.imageUrl} /><div><span className="pill">{item.type}</span><h3>{item.bakerName}</h3><p>{item.caption}</p></div></article>)}{!feed.length ? <EmptyState title="No creator posts found" /> : null}</div></div>;
}
