import { Bell, Heart, LogOut, Menu, Search, ShoppingBag, UserRound, X, ChevronDown } from 'lucide-react';
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    if (role === Role.CUSTOMER) cartApi.get().then((cart) => setCartCount(cart?.items?.length || 0)).catch(() => {});
  ...
  return (
    <header className="site-header">
      ...
      <form className="search-form" onSubmit={submitSearch}>
        <Search size={17} />
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search cakes, breads..." />
      </form>
